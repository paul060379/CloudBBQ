...
const music={paused:true,volume:0,plays:0,pauses:0,play(){this.paused=false;this.plays++;return Promise.resolve()},pause(){this.paused=true;this.pauses++}};const document={querySelector:s=>s==='#bg-music'?music:null};const window={AudioContext:Context,dispatchEvent(){}};const navigator={audioSession:{type:'auto'}};vm.runInNewContext(fs.readFileSync('audio.js','utf8'),{window,document,navigator,Event,Math,Float32Array});return{api:window.PicnicAudio,navigator,music,get context(){return context},get tones(){return tones},resume:()=>resolveResume(),reject:()=>rejectResume(new Error('blocked'))}}
...
test('background music follows the game and the sound switch',async()=>{const h=audioHarness();h.api.music(true);const pending=h.api.setEnabled(true);assert.equal(h.music.plays,1);h.resume();assert.equal(await pending,true);h.api.music(false);assert.equal(h.music.pauses,1);assert.equal(h.music.volume,.22)});
...
test('the image goes out with a caption and the game link, shown in the dialog too',async()=>{const h=shareHarness(true,'zh');await h.api.open({...record,player:'阿肉',score:120});
 ...
 assert.match(h.e['#share-caption-text'].textContent,/https:\/\/github\.com\/paul060379\/CloudBBQ/);
